...
{
  fprint_vmcs(out, VMCS_VPID);
  fprint_vmcs(out, VMCS_CTRL_POSTED_INT_N_VECTOR);
  fprint_vmcs(out, VMCS_CTRL_EPTP_INDEX);
  fprint_vmcs(out, VMCS_GUEST_ES);
  fprint_vmcs(out, VMCS_GUEST_CS);
  fprint_vmcs(out, VMCS_GUEST_SS);
  fprint_vmcs(out, VMCS_GUEST_DS);
  fprint_vmcs(out, VMCS_GUEST_FS);
  fprint_vmcs(out, VMCS_GUEST_GS);
  fprint_vmcs(out, VMCS_GUEST_LDTR);
  fprint_vmcs(out, VMCS_GUEST_TR);
  fprint_vmcs(out, VMCS_GUEST_INT_STATUS);
  fprint_vmcs(out, VMCS_HOST_ES);
  fprint_vmcs(out, VMCS_HOST_CS);
  fprint_vmcs(out, VMCS_HOST_SS);
  fprint_vmcs(out, VMCS_HOST_DS);
  fprint_vmcs(out, VMCS_HOST_FS);
  fprint_vmcs(out, VMCS_HOST_GS);
  fprint_vmcs(out, VMCS_HOST_TR);
  fprint_vmcs(out, VMCS_CTRL_IO_BITMAP_A);
  fprint_vmcs(out, VMCS_CTRL_IO_BITMAP_B);
  fprint_vmcs(out, VMCS_CTRL_MSR_BITMAPS);
  fprint_vmcs(out, VMCS_CTRL_VMEXIT_MSR_STORE_ADDR);
  fprint_vmcs(out, VMCS_CTRL_VMEXIT_MSR_LOAD_ADDR);
  fprint_vmcs(out, VMCS_CTRL_VMENTRY_MSR_LOAD_ADDR);
  fprint_vmcs(out, VMCS_CTRL_EXECUTIVE_VMCS_PTR);
  fprint_vmcs(out, VMCS_CTRL_TSC_OFFSET);
  fprint_vmcs(out, VMCS_CTRL_VIRTUAL_APIC);
  fprint_vmcs(out, VMCS_CTRL_APIC_ACCESS);
  fprint_vmcs(out, VMCS_CTRL_POSTED_INT_DESC_ADDR);
  fprint_vmcs(out, VMCS_CTRL_VMFUNC_CTRL);
  fprint_vmcs(out, VMCS_CTRL_EPTP);
  fprint_vmcs(out, VMCS_CTRL_EOI_EXIT_BITMAP_0);
  fprint_vmcs(out, VMCS_CTRL_EOI_EXIT_BITMAP_1);
  fprint_vmcs(out, VMCS_CTRL_EOI_EXIT_BITMAP_2);
  fprint_vmcs(out, VMCS_CTRL_EOI_EXIT_BITMAP_3);
  fprint_vmcs(out, VMCS_CTRL_EPTP_LIST_ADDR);
  fprint_vmcs(out, VMCS_CTRL_VMREAD_BITMAP_ADDR);
  fprint_vmcs(out, VMCS_CTRL_VMWRITE_BITMAP_ADDR);
  fprint_vmcs(out, VMCS_CTRL_VIRT_EXC_INFO_ADDR);
  fprint_vmcs(out, VMCS_CTRL_XSS_EXITING_BITMAP);
  fprint_vmcs(out, VMCS_GUEST_PHYSICAL_ADDRESS);
  fprint_vmcs(out, VMCS_GUEST_LINK_POINTER);
  fprint_vmcs(out, VMCS_GUEST_IA32_DEBUGCTL);
  fprint_vmcs(out, VMCS_GUEST_IA32_PAT);
  fprint_vmcs(out, VMCS_GUEST_IA32_EFER);
  fprint_vmcs(out, VMCS_GUEST_IA32_PERF_GLOBAL_CTRL);
  fprint_vmcs(out, VMCS_GUEST_PDPTE0);
  fprint_vmcs(out, VMCS_GUEST_PDPTE1);
  fprint_vmcs(out, VMCS_GUEST_PDPTE2);
  fprint_vmcs(out, VMCS_GUEST_PDPTE3);
  fprint_vmcs(out, VMCS_HOST_IA32_PAT);
  fprint_vmcs(out, VMCS_HOST_IA32_EFER);
  fprint_vmcs(out, VMCS_HOST_IA32_PERF_GLOBAL_CTRL);
  fprint_vmcs(out, VMCS_CTRL_PIN_BASED);
  fprint_vmcs(out, VMCS_CTRL_CPU_BASED);
  fprint_vmcs(out, VMCS_CTRL_EXC_BITMAP);
  fprint_vmcs(out, VMCS_CTRL_PF_ERROR_MASK);
  fprint_vmcs(out, VMCS_CTRL_PF_ERROR_MATCH);
  fprint_vmcs(out, VMCS_CTRL_CR3_COUNT);
  fprint_vmcs(out, VMCS_CTRL_VMEXIT_CONTROLS);
  fprint_vmcs(out, VMCS_CTRL_VMEXIT_MSR_STORE_COUNT);
  fprint_vmcs(out, VMCS_CTRL_VMEXIT_MSR_LOAD_COUNT);
  fprint_vmcs(out, VMCS_CTRL_VMENTRY_CONTROLS);
  fprint_vmcs(out, VMCS_CTRL_VMENTRY_MSR_LOAD_COUNT);
  fprint_vmcs(out, VMCS_CTRL_VMENTRY_IRQ_INFO);
  fprint_vmcs(out, VMCS_CTRL_VMENTRY_EXC_ERROR);
  fprint_vmcs(out, VMCS_CTRL_VMENTRY_INSTR_LEN);
  fprint_vmcs(out, VMCS_CTRL_TPR_THRESHOLD);
  fprint_vmcs(out, VMCS_CTRL_CPU_BASED2);
  fprint_vmcs(out, VMCS_CTRL_PLE_GAP);
  fprint_vmcs(out, VMCS_CTRL_PLE_WINDOW);
  fprint_vmcs(out, VMCS_RO_INSTR_ERROR);
  fprint_vmcs(out, VMCS_RO_EXIT_REASON);
  fprint_vmcs(out, VMCS_RO_VMEXIT_IRQ_INFO);
  fprint_vmcs(out, VMCS_RO_VMEXIT_IRQ_ERROR);
  fprint_vmcs(out, VMCS_RO_IDT_VECTOR_INFO);
  fprint_vmcs(out, VMCS_RO_IDT_VECTOR_ERROR);
  fprint_vmcs(out, VMCS_RO_VMEXIT_INSTR_LEN);
  fprint_vmcs(out, VMCS_RO_VMX_INSTR_INFO);
  fprint_vmcs(out, VMCS_GUEST_ES_LIMIT);
  fprint_vmcs(out, VMCS_GUEST_CS_LIMIT);
  fprint_vmcs(out, VMCS_GUEST_SS_LIMIT);
  fprint_vmcs(out, VMCS_GUEST_DS_LIMIT);
  fprint_vmcs(out, VMCS_GUEST_FS_LIMIT);
  fprint_vmcs(out, VMCS_GUEST_GS_LIMIT);
  fprint_vmcs(out, VMCS_GUEST_LDTR_LIMIT);
  fprint_vmcs(out, VMCS_GUEST_TR_LIMIT);
  fprint_vmcs(out, VMCS_GUEST_GDTR_LIMIT);
  fprint_vmcs(out, VMCS_GUEST_IDTR_LIMIT);
  fprint_vmcs(out, VMCS_GUEST_ES_AR);
  fprint_vmcs(out, VMCS_GUEST_CS_AR);
  fprint_vmcs(out, VMCS_GUEST_SS_AR);
  fprint_vmcs(out, VMCS_GUEST_DS_AR);
  fprint_vmcs(out, VMCS_GUEST_FS_AR);
  fprint_vmcs(out, VMCS_GUEST_GS_AR);
  fprint_vmcs(out, VMCS_GUEST_LDTR_AR);
  fprint_vmcs(out, VMCS_GUEST_TR_AR);
  fprint_vmcs(out, VMCS_GUEST_IGNORE_IRQ);
  fprint_vmcs(out, VMCS_GUEST_ACTIVITY_STATE);
  fprint_vmcs(out, VMCS_GUEST_SMBASE);
  fprint_vmcs(out, VMCS_GUEST_IA32_SYSENTER_CS);
  fprint_vmcs(out, VMCS_GUEST_VMX_TIMER_VALUE);
  fprint_vmcs(out, VMCS_HOST_IA32_SYSENTER_CS);
  fprint_vmcs(out, VMCS_CTRL_CR0_MASK);
  fprint_vmcs(out, VMCS_CTRL_CR4_MASK);
  fprint_vmcs(out, VMCS_CTRL_CR0_SHADOW);
  fprint_vmcs(out, VMCS_CTRL_CR4_SHADOW);
  fprint_vmcs(out, VMCS_CTRL_CR3_VALUE0);
  fprint_vmcs(out, VMCS_CTRL_CR3_VALUE1);
  fprint_vmcs(out, VMCS_CTRL_CR3_VALUE2);
  fprint_vmcs(out, VMCS_CTRL_CR3_VALUE3);
  fprint_vmcs(out, VMCS_RO_EXIT_QUALIFIC);
  fprint_vmcs(out, VMCS_RO_IO_RCX);
  fprint_vmcs(out, VMCS_RO_IO_RSI);
  fprint_vmcs(out, VMCS_RO_IO_RDI);
  fprint_vmcs(out, VMCS_RO_IO_RIP);
  fprint_vmcs(out, VMCS_RO_GUEST_LIN_ADDR);
  fprint_vmcs(out, VMCS_GUEST_CR0);
  fprint_vmcs(out, VMCS_GUEST_CR3);
  fprint_vmcs(out, VMCS_GUEST_CR4);
  fprint_vmcs(out, VMCS_GUEST_ES_BASE);
  fprint_vmcs(out, VMCS_GUEST_CS_BASE);
  fprint_vmcs(out, VMCS_GUEST_SS_BASE);
  fprint_vmcs(out, VMCS_GUEST_DS_BASE);
  fprint_vmcs(out, VMCS_GUEST_FS_BASE);
  fprint_vmcs(out, VMCS_GUEST_GS_BASE);
  fprint_vmcs(out, VMCS_GUEST_LDTR_BASE);
  fprint_vmcs(out, VMCS_GUEST_TR_BASE);
  fprint_vmcs(out, VMCS_GUEST_GDTR_BASE);
  fprint_vmcs(out, VMCS_GUEST_IDTR_BASE);
  fprint_vmcs(out, VMCS_GUEST_DR7);
  fprint_vmcs(out, VMCS_GUEST_RSP);
  fprint_vmcs(out, VMCS_GUEST_RIP);
  fprint_vmcs(out, VMCS_GUEST_RFLAGS);
  fprint_vmcs(out, VMCS_GUEST_DEBUG_EXC);
  fprint_vmcs(out, VMCS_GUEST_SYSENTER_ESP);
  fprint_vmcs(out, VMCS_GUEST_SYSENTER_EIP);
  fprint_vmcs(out, VMCS_HOST_CR0);
  fprint_vmcs(out, VMCS_HOST_CR3);
  fprint_vmcs(out, VMCS_HOST_CR4);
  fprint_vmcs(out, VMCS_HOST_FS_BASE);
  fprint_vmcs(out, VMCS_HOST_GS_BASE);
  fprint_vmcs(out, VMCS_HOST_TR_BASE);
  fprint_vmcs(out, VMCS_HOST_GDTR_BASE);
  fprint_vmcs(out, VMCS_HOST_IDTR_BASE);
  fprint_vmcs(out, VMCS_HOST_IA32_SYSENTER_ESP);
  fprint_vmcs(out, VMCS_HOST_IA32_SYSENTER_EIP);
  fprint_vmcs(out, VMCS_HOST_RSP);
  fprint_vmcs(out, VMCS_HOST_RIP);
  fprint_vmcs(out, VMCS_MAX);
}
